...
if(!sistemlog) return message.reply('Lütfen Bir Log kanalı ayarlayınız! **f!sistemlog #kanal** şeklinde ayarlayabilirsiniz.')
  ...
  if (args[0] == "kapat") {
    if (db.has(`antiraidK_${message.guild.id}`) === false) {
      return message.channel.send(
        "Anti-raid açılmamış. Açmak için **f!anti-raid aç**"
      );
    }
  ...
  if (!args[0])
    return message.reply(
      "Lütfen geçerli işlem girin. Örnek: **f!anti-raid aç/kapat**"
    );
};
exports.conf = {
  enabled: true,
