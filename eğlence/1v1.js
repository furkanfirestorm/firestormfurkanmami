const Discord = require('discord.js');//geziyon mu .D
...
exports.run = async (client, message, args) => {
  ...
  this.fighting = new Set();
        ...
        if (!opponent) return message.reply(":x: Oynamak istediğin kişiyi etiketlemelisiniz.")
  
  if (opponent.bot) return message.reply(':x: Botlar ile düello yapamazsın dostum.');
  if (opponent.id === message.author.id) return message.reply(':x: Kendin ile düello atamazsın dostum.');
                if (this.fighting.has(message.channel.id)) return message.reply('Kanal sayısına göre savaş yapabilirsin.');
                this.fighting.add(message.channel.id);
                try {
                        if (!opponent.bot) {
                await message.channel.send(`${opponent}, düello isteği geldi. Düello'yu kabul ediyor musun? (\`evet\` veya \`hayır\`)`);
...
await message.channel.send(`${user}, Çoook uzak galaksilerden gelen ultra sonik enerjiyi yeterli miktarda toplayamadığın için ultra güç kullanamadın!`);
                                        ...
                                        await message.channel.send(`${user}, kaçtı! Korkak! hahaha`);
                                        forfeit();
                                        break;
                                } else {
